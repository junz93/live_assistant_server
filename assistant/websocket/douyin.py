...
roomStore = None
liveRoomTitle = None
gift_info_file = open('./logs/gift_info.json', 'a', encoding='utf-8')
stop_event = threading.Event()
# All ws connections with douyin danmu server
ws_connections = []
# The next ws connection to restart
next_ws_connection_index = 0
...
def unPackWebcastCommonTextMessage(data):
    ...
    # logging.info('[unPackWebcastCommonTextMessage] [] ｜ ' + log)
    return data
...
def unPackWebcastUpdateFanTicketMessage(data):
    ...
    log = json.dumps(data_dict, ensure_ascii=False)
    # logging.info('[UpdateFanTicket] ｜ ' + log)
    # logging.info('[unPackWebcastUpdateFanTicketMessage] [] ｜ ' + log)
    return data
...
def unPackWebcastRoomUserSeqMessage(data):
    ...
    log = json.dumps(data_dict, ensure_ascii=False)
    # logging.info('[直播间观众信息] ｜ ' + log)
    # logging.info('[unPackWebcastRoomUserSeqMessage] [] ｜ ' + log)
    return data
...
def unPackWebcastSocialMessage(data):
    ...
    log = json.dumps(data_dict, ensure_ascii=False)
    # logging.info('[➕直播间关注消息] ｜ ' + socialMessage.user.nickName + ' 关注了直播间')
    # logging.info('[unPackWebcastSocialMessage] [➕直播间关注消息] ｜ ' + log)
...
def unPackWebcastChatMessage(data):
    ...
    with processed_message_lock:
        if chatMessage.common.msgId in processed_message_ids:
            logging.info(f'Skipped duplicate message {chatMessage.common.msgId}')
            return data
        processed_message_ids.add(chatMessage.common.msgId)

    # Aliyun 文本审核
    if not content_censorship.check_text(chatMessage.content):
        return data

    gol.add_danmu_list(chatMessage)
    # logging.info(f"[📧直播间弹幕消息] ｜ {chatMessage.user.nickName}: {chatMessage.content}，各优先级消息还剩余：{gol.get_all_list_num()}")
...
def unPackWebcastGiftMessage(data):
    ...
    with processed_message_lock:
        if giftMessage.common.msgId in processed_message_ids:
            logging.info(f'Skipped duplicate message {giftMessage.common.msgId}')
            return data
        processed_message_ids.add(giftMessage.common.msgId)
    ...
    # logging.info(
        # f'[🎁直播间礼物消息] ｜ {giftMessage.common.describe}  金额：{giftMessage.gift.diamondCount} x {giftMessage.comboCount} = {gift_amount}，各优先级消息还剩余：{gol.get_all_list_num()}')
    gol.add_gift_list(giftMessage)
...
def unPackWebcastMemberMessage(data):
    ...
    with processed_message_lock:
        if memberMessage.common.msgId in processed_message_ids:
            logging.info(f'Skipped duplicate message {memberMessage.common.msgId}')
            return data
        processed_message_ids.add(memberMessage.common.msgId)

    gol.add_enter_list(memberMessage)
...
def unPackWebcastLikeMessage(data):
    ...
    with processed_message_lock:
        if likeMessage.common.msgId in processed_message_ids:
            logging.info(f'Skipped duplicate message {likeMessage.common.msgId}')
            return data
        processed_message_ids.add(likeMessage.common.msgId)

    gol.add_like_list(likeMessage)
...
def unPackMatchAgainstScoreMessage(data):
    ...
    log = json.dumps(data_dict, ensure_ascii=False)
    # logging.info('[unPackMatchAgainstScoreMessage] [🤷不知道是啥的消息] ｜ ' + log)
    return data
...
def sendAck(ws, logId, internalExt):
    ...
    # logging.info('[sendAck] [🌟发送Ack] ====> 房间🏖标题【' + liveRoomTitle + '】')
...
def onOpen(ws):
    thread = threading.Thread(target=ping, args=[ws])
    thread.start()
    ...
    try:
        while True:
            ...
            if n == 10:
                ...
                data = obj.SerializeToString()
                ws.send(data, websocket.ABNF.OPCODE_BINARY)
                logging.info('[💗发送ping心跳] 房间标题【' + liveRoomTitle + '】')
            else:
                n += 1
            time.sleep(1)
...
def wssServerStart(roomId):
    global liveRoomId, userUniqueId
    liveRoomId = roomId
    websocket.enableTrace(False)
    webSocketUrl = 'wss://webcast3-ws-web-hl.douyin.com/webcast/im/push/v2/?app_name=douyin_web&version_code=180800&webcast_sdk_version=1.3.0&update_version_code=1.3.0&compress=gzip&internal_ext=internal_src:dim|wss_push_room_id:'+liveRoomId+'|wss_push_did:'+userUniqueId+'|dim_log_id:202302171547011A160A7BAA76660E13ED|fetch_time:1676620021641|seq:1|wss_info:0-1676620021641-0-0|wrds_kvs:WebcastRoomStatsMessage-1676620020691146024_WebcastRoomRankMessage-1676619972726895075_AudienceGiftSyncData-1676619980834317696_HighlightContainerSyncData-2&cursor=t-1676620021641_r-1_d-1_u-1_h-1&host=https://live.douyin.com&aid=6383&live_id=1&did_rule=3&debug=false&endpoint=live_pc&support_wrds=1&im_path=/webcast/im/fetch/&user_unique_id='+userUniqueId+'&device_platform=web&cookie_enabled=true&screen_width=1440&screen_height=900&browser_language=zh&browser_platform=MacIntel&browser_name=Mozilla&browser_version=5.0%20(Macintosh;%20Intel%20Mac%20OS%20X%2010_15_7)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/110.0.0.0%20Safari/537.36&browser_online=true&tz_name=Asia/Shanghai&identity=audience&room_id='+liveRoomId+'&heartbeatDuration=0&signature=00000000'
...
def start_danmu_handler(room_id, num_threads=1):
    ...
    global ttwid, roomStore, liveRoomId, userUniqueId, liveRoomTitle
    ...
    userUniqueId = res['app']['odin']['user_unique_id']
    logging.info(f'Retrieved information for the live room. Room ID: {liveRoomId}, user unique ID: {userUniqueId}')
    
    for i in range(num_threads):
        thread = threading.Thread(target=wssServerStart, args=[liveRoomId], daemon=True)
        thread.start()

    # Runs forever
    # schedule_restart_websocket(num_threads)


def schedule_restart_websocket(num_ws_connections=1):
    global next_ws_connection_index

    while len(ws_connections) < num_ws_connections:
        time.sleep(0.1)
    ...
    while True:
        ...
        ws_connections[next_ws_connection_index].close()
        logging.info(f'Closed the websocket connection {next_ws_connection_index}')
        next_ws_connection_index = (next_ws_connection_index + 1) % len(ws_connections)
